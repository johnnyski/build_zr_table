/* get_radar_data_over_gauge.h
 */


#ifndef __GET_RADAR_DATA_OVER_GAUGE_H__
#define __GET_RADAR_DATA_OVER_GAUGE_H__ 1
...
typedef struct _zrc {
  ...
  float         z;            /* Z: reflectivity in dBz */
} zc_t;

typedef struct _height_info {
  float         height;   /* Height in km. */
  zc_t          *zc;      /* nzc entries of 2 tuples(Z, raintype).
